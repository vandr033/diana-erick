...
export function ResponseFilters({ initial }: { initial: Record<string, string> }) {
  ...
  return <form className="response-filters" method="get">
    ...
    <div className="admin-field"><label htmlFor="filter-attendance">Asistencia</label><select id="filter-attendance" name="attendance" value={filters.attendance || ""} onChange={(event) => update("attendance", event.target.value)}><option value="">Todas</option><option value="si">Asiste</option><option value="no">No asiste</option></select></div>
