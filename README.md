...
- `/admin/whatsapp` — importar invitados, abrir mensajes manuales de WhatsApp y copiar enlaces.
...
En `/admin/whatsapp`, importa la primera hoja de un archivo `.xlsx`, `.xls` o `.csv`. Se valida antes de agregar personas. Las columnas requeridas son:
...
No se conecta ninguna API de WhatsApp ni se ejecuta un worker. En cada fila importada, el botón **Enviar por WhatsApp** abre `wa.me` con el teléfono, el nombre, el enlace y el mensaje ya preparados. Solo debes revisar y presionar enviar en WhatsApp Web.

El texto predeterminado se edita en **Contenido → Mensaje de invitación** y admite `{name}` y `{link}`. Un `custom_message` en el Excel reemplaza el texto predeterminado para esa persona.
...
- No hay correos, SMS, APIs de WhatsApp ni recordatorios automáticos.
